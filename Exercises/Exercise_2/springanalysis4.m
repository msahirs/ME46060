...
% Visualization of first eigenfrequency
...
% Contour plot of first eigenfrequency
subplot(221)
contour(D, d, funk, ShowText="on")
xlabel('D (m)'), ylabel('d (m)'), title('Contours of first eigenfrequency(Hz)')
grid

% Plot of first eigenfrequency for constant d(1) = 0.002 
subplot(222)
plot(D, funk(1,:))
xlabel('D (m)'), ylabel('first eigenfrequency (Hz)'), ...
   title('Wire diameter 0.002(m)') 
grid

%Plot of first eigenfrequency for constant D(1) = 0.02(m) 
subplot(223)
plot(d, funk(:,1))
xlabel('d (m)'), ylabel('first eigenfrequency (Hz)'), ...
...
%Plot of first eigenfrequency for constant D(21) = 0.04(m) 
subplot(224)
plot(d, funk(:,21))
xlabel('d (m)'), ylabel('first eigenfrequency (Hz)'), ...
